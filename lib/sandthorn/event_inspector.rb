...
module Sandthorn
  module EventInspector
    ...
    def get_unsaved_events(event_name)
      aggregate_events.select { |e| e[:event_name] == event_name.to_s  }
    end

    def get_saved_events(event_name)
      saved_events = Sandthorn.get_aggregate_events aggregate_id, self.class
