  ...
  module AggregateRoot
    ...
    describe "event data" do
      ...
      context "after find" do
        ...
        it "should set the old_value on the event" do
          ...
          expect(dirty_object_after_find.aggregate_events.last[:event_data]["name"][:old_value]).to eql "old_value"
        end
      ...
      context "old_value should be set" do
        ...
        it "should set the old_value on the event" do
          dirty_object.change_name "new_name"
          expect(dirty_object.aggregate_events.last[:event_data]["name"][:old_value]).to eql "old_value"
        end

        it "should not change aggregate_id" do
          dirty_object.change_name "new_name"
          expect(dirty_object.aggregate_events.last[:event_data]["attribute_name"]).not_to eql "aggregate_id"
        end

        it "should not change sex attribute if sex method is not runned" do
          dirty_object.change_name "new_name"
          dirty_object.aggregate_events.each do |event|
            expect(event[:event_data]["sex"].nil?).to be_truthy
          end
        end
        ...
        it "should set old_value and new_value on sex change" do
          ...
          expect(dirty_object.aggregate_events.last[:event_data]["sex"][:old_value]).to eql "hen"
          expect(dirty_object.aggregate_events.last[:event_data]["sex"][:new_value]).to eql "shemale"
        end
      end
    end
    ...
    context "events should be created event if no state change is made" do
      ...
      it "should have event_data set to empty hash" do
        expect(dirty_object.aggregate_events.first[:event_data]).to eql({})
      end
