    ...
    describe "when making a change on a aggregate" do
      ...
      context "when changing name (attr_reader)" do
        ...
        it "should generate one event on new" do
          expect(dirty_obejct.events.length).to eql 1
        end

        it "should generate 2 events new and change_name" do
          dirty_obejct.change_name "new_name"
          expect(dirty_obejct.events.length).to eql 2
        end
      end
      ...
      context "save" do
        it "should not have events on aggregete after save" do
          expect(dirty_obejct.save.events.length).to eql 0
        end

        it "should have originating_version == 0 pre save" do
          expect(dirty_obejct.originating_version).to eql 0
        end
    ...
    describe "event data" do
      ...
      context "after find" do
        ...
        it "should set the old_value on the event" do
          ...
          expect(dirty_obejct_after_find.events.last[:event_args][:attribute_deltas].first[:old_value]).to eql "old_value"
        end
      ...
      context "old_value should be set" do
        ...
        it "should set the old_value on the event" do
          dirty_obejct.change_name "new_name"
          expect(dirty_obejct.events.last[:event_args][:attribute_deltas].first[:old_value]).to eql "old_value"
        end

        it "should not change aggregate_id" do
          dirty_obejct.change_name "new_name"
          expect(dirty_obejct.events.last[:event_args][:attribute_deltas].last[:attribute_name]).not_to eql "aggregate_id"
        end

        it "should not change sex attribute if sex method is not runned" do
          dirty_obejct.change_name "new_name"
          dirty_obejct.events.each do |event|
        ...
        it "should set old_value and new_value on sex change" do
          ...
          expect(dirty_obejct.events.last[:event_args][:attribute_deltas].first[:old_value]).to eql "hen"
          expect(dirty_obejct.events.last[:event_args][:attribute_deltas].first[:new_value]).to eql "shemale"
        end
      end
    end
  end
